...
* We use K3s - a lightweight Kubernetes distribution by Rancher. It is characterized by a small "footprint", while maintaining full Kubernetes functionality. So this is a kind of Kubernetes distribution, not a stripped down version of it. The preferred area of application of K3s are environments with a limited amount of computing resources (e.g. so-called edge solutions in the network). Our Raspberry Pi-based environment fits perfectly into this scenario. The controller (master) node modules occupy a total of approximately 512MB RAM, and the worker node modules require less than 50MB RAM. Implementation bias from "vanilla Kubernetes" consists in particular in that the Kubernetes functions of a given type of node (master/worker) are implemented in one process (e.g. kubelet, kube-proxy and flanneld modules in a worker node); while in K8s, individual functions are implemented as independent pods/processes (ref. https://traefik.io/glossary/k3s-explained/). 
...
**_5G Core - deploying 5G Core (free5GC) platform + RAN network simulator_ (interested persons - please contact me on priv)**
  * installation and operation check (e.g. ping 8.8.8.8) of the open source 5G platform (free5GC) in the following configuration: (1) 5G core control plane on the K3s/RaspberryPi cluster, (2) 5G core UPF on the K3s/separate VM/VirtualBox cluster, ( 3) RAN network emulator (5G core side, UE+gNodeB equivalent) on a separate VM/VirtualBox
