...
4. **WAŻNE: obecna wersja skryptu install.sh wymaga**, aby instalacja była przeprowadzona w docelowym segmencie L2 sieci i niemożliwa jest instalacja przez VPN. Inymi słowy, maszyna management i klaster muszą pracować podcza instalacji w tym samym segmencie L2. Wynika to z tego, że narzędzie nmap, wykorzystywane w naszym przypadku do pobrania adresów MAC, musi mieć bezpośredni dostęp do właściwego segmentu L2 sieci. Rozwiązania typu L3 VPN, w tym Zero-Tier, z definicji blokują dostęp do L2 odległych stron. Dostęp L3 VPN mógłby zostać wykorzystany, jednak pod warunkiem zrezygnowqania z użycia nmap w install.sh i ręcznego wypełnienia pliku config (obecnie tworzonego i wypełnianego przez install.sh) właściwymi adresmi IP węzłów klastra przed uruchomieniam skryptu install.sh. (Zastosowanie L2 VPN dałoby możliwosć użycia install.sh bez żadnych zmian.)
