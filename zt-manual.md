...
## Uwagi ogólne
1. Poniższy opis sporządzono przy założeniu, że ZeroTier będzie zainstalowane na jednej z malinek naszego klastra. W takiej konfiguracji przynajmniej ta malinka powinna cały czas pracować. Pozostałe malinki mogą być wyłączane na czas sensownie długich przerw w pracy - dla oszczędzania malinek. Niestety, wyłączenie malinek (`shutdown`) nie zatrzymuje wentylatorów w nakładkach PoE - wentylatory te można wyłączyć jedynie odłączając zasilanie całego rutera TP-Link (w czterech klastrach nakładka posiada mikroprzełącznik, którym ręcznie można wyłączyć wentylator na stałe). Sposób zdalnego włączenia klastra podano w następnym punkcie Uwag ogólnych.
